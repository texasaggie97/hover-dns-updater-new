...
class HoverConfig(object):
    def __init__(self, args):
        ...
        self.LOGFILE = (
            self._config["logfile"] if args.log_file is None else args.log_file
        )
        self.SERVICE = (
            self._config["run-as-service"] if args.service is None else args.service
        )
        self.POLLTIME = (
            self._config["poll-time"] if args.poll_time is None else args.poll_time
        )
...
class HoverAPI(object):
    ...
    def update(self):
        ...
        for dns_id in self._current_dns_ips:
            ...
            if self._current_dns_ips[dns_id] != current_external_ip:
                logging.info(
                    "    Updating DNS entry for {0} to {1}".format(
    ...
    def call(self, method, resource, data=None):
        logging.debug(
            "method={0}, resource={1}, data={2}".format(method, resource, str(data))
        )
        self.check_auth()
...
def _parse_args(argv):
    ...
    config_group.add_argument(
        "-c",
        "--config-file",
        default="hover-update.cfg",
        action="store",
        help="Config file, in json format. If it does not exist, one will be created with default values.",
    )
    config_group.add_argument(
        "--service", default=False, action="store_true", help="Run as a service"
    )
    config_group.add_argument(
        "--poll-time",
        default=None,
        action="store",
        help="Time in seconds to sleep between polling the ip address",
    )
    ...
    verbosity_group.add_argument(
        "--test", action="store_true", default=False, help="Run doctests then quit."
    )
    verbosity_group.add_argument(
        "--log-file",
        action="store",
